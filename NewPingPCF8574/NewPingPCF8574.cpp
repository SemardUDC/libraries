...
//
// See NewPingPCF8574.h for license, purpose, syntax, version history, links, etc.
// ---------------------------------------------------------------------------

#include "NewPingPCF8574.h"

// ---------------------------------------------------------------------------
// NewPingPCF8574 constructor
// ---------------------------------------------------------------------------

NewPingPCF8574::NewPingPCF8574(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance, PCF8574 &pcf)
    : _pcf(pcf)
{
#if DO_BITWISE == true
...
unsigned int NewPingPCF8574::ping(unsigned int max_cm_distance)
{
...
#else
    while (!_pcf.read(_echoPin)) // Wait for the ping echo.
#endif
        if (micros() > _max_time)
...
unsigned long NewPingPCF8574::ping_cm(unsigned int max_cm_distance)
{
    unsigned long echoTime = NewPingPCF8574::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
    return (echoTime / US_ROUNDTRIP_CM); // Call the ping method and returns the distance in centimeters (no rounding).
#else
    return NewPingPCF8574Convert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
#endif
}

unsigned long NewPingPCF8574::ping_in(unsigned int max_cm_distance)
{
    unsigned long echoTime = NewPingPCF8574::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
    return (echoTime / US_ROUNDTRIP_IN); // Call the ping method and returns the distance in inches (no rounding).
#else
    return NewPingPCF8574Convert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
#endif
}

unsigned long NewPingPCF8574::ping_median(uint8_t it, unsigned int max_cm_distance)
{
...
boolean NewPingPCF8574::ping_trigger()
{
#if DO_BITWISE == true
    ...
    *_triggerMode &= ~_triggerBit; // Set trigger pin to input (when using one Arduino pin, this is technically setting the echo pin to input as both are tied to the same Arduino pin).
    ...
    delayMicroseconds(4);            // Wait for pin to go low.
    _pcf.write(_triggerPin, HIGH); // Set trigger pin high, this tells the sensor to send out a ping.
    delayMicroseconds(10);           // Wait long enough for the sensor to realize the trigger pin is high. Sensor specs say to wait 10uS.
...
#if URM37_ENABLED == true
    if (!_pcf.read(_echoPin))
        return false;                                       // Previous ping hasn't finished, abort.
    _max_time = micros() + _maxEchoTime + MAX_SENSOR_DELAY; // Maximum time we'll wait for ping to start (most sensors are <450uS, the SRF06 can take up to 34,300uS!)
    while (_pcf.read(_echoPin))                           // Wait for ping to start.
        if (micros() > _max_time)
            return false;                                                                                             // Took too long to start, abort.
#else
    if (_pcf.read(_echoPin))
...
void NewPingPCF8574::set_max_distance(unsigned int max_cm_distance)
{
#if ROUNDING_ENABLED == false
...
void NewPingPCF8574::ping_timer(void (*userFunc)(void), unsigned int max_cm_distance)
{
    if (max_cm_distance > 0)
        set_max_distance(max_cm_distance); // Call function to set a new max sensor distance.
...
boolean NewPingPCF8574::check_timer()
{
    if (micros() > _max_time)
    {                 // Outside the time-out limit.
...
void NewPingPCF8574::timer_us(unsigned int frequency, void (*userFunc)(void))
{
...
void NewPingPCF8574::timer_ms(unsigned long frequency, void (*userFunc)(void))
{
    intFunc = NewPingPCF8574::timer_ms_cntdwn; // Timer events are sent here once every ms till user's frequency is reached.
    ...
    TIMSK4 = (1 << TOIE4);                     // Enable Timer4 interrupt.
#elif defined(__arm__) && defined(TEENSYDUINO) // Timer for Teensy 3.x
    itimer.begin(NewPingPCF8574::timer_ms_cntdwn, 1000); // Set timer to 1ms (1000 uS).
#elif defined(__arm__) && defined(PARTICLE)    // Timer for Particle
    itimer.begin(NewPingPCF8574::timer_ms_cntdwn, 1000, uSec); // Set timer to 1ms (1000 uS).
#else
...
void NewPingPCF8574::timer_stop()
...
void NewPingPCF8574::timer_setup()
{
#if defined(__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
...
void NewPingPCF8574::timer_ms_cntdwn()
{
    if (!_ms_cnt--)
...
unsigned int NewPingPCF8574::convert_cm(unsigned int echoTime)
{
#if ROUNDING_ENABLED == false
...
unsigned int NewPingPCF8574::convert_in(unsigned int echoTime)
{
#if ROUNDING_ENABLED == false
